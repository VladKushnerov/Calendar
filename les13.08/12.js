...
container.insertAdjacentHTML('beforeend',`
    <div>
    <h3>${hardwareStore[myURL].name.name}</h3>
    <p>${hardwareStore[myURL].name.description}</p>
    <span>${hardwareStore[myURL].name.price}</span>
    <button class='btn'>Повернутися</button>
    </div>
    `)
const btn = document.getElementById('btn')
btn.addEventListener('click',()=>location.href=`http://127.0.0.1:5500/`)